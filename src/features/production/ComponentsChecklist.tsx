...
import { useToggleWorkOrderItemCompleted, type WorkOrderWithItems } from "./api";
...
export function ComponentsChecklist({ wo }: Props) {
  const toggle = useToggleWorkOrderItemCompleted();
  const total = wo.items.length;
  const done = wo.items.filter((it) => it.is_completed).length;
  const pct = total === 0 ? 0 : Math.round((done / total) * 100);
  const allDone = total > 0 && done === total;

  const handleToggle = async (id: string, next: boolean) => {
    try {
      await toggle.mutateAsync({ id, is_completed: next });
    } catch (err) {
    ...
    <div className="space-y-4">
      <div className="space-y-2">
        <div className="flex items-center justify-between text-sm">
          <span className="text-muted-foreground">Progreso de unidades</span>
      ...
      {allDone && (
        <Alert className="border-primary/40 bg-primary/10">
          ...
          <AlertTitle>Producción completa</AlertTitle>
          <AlertDescription>
            Todas las unidades del lote están marcadas como completadas.
          </AlertDescription>
        </Alert>
      )}
