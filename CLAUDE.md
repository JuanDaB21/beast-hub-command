...
- [x] FASE 11 — Guía obligatoria, fusión COD→Logística, conciliación por vía de cobro y flete unificado (rama `claude/logistics-guide-cod-merge`):
  ...
  - ⚠️ El espejo automático del flete en el libro quedó **revertido en FASE 12**.

- [x] FASE 12 — Libro manual + pago de prendas sin duplicados (rama `claude/manual-ledger-unit-payments`):
  - **Libro 100% manual**: el servidor solo escribe en `financial_transactions` desde `POST /finance` (manual) y `unit-payments.ts` (`reference_type='unit_payment'`, única excepción automática). Se quitó el espejo de `shipping_cost` en `PATCH /orders/:id` y los asientos de merma / flete RMA en `returns/:id/resolve`. **Migración `026_manual_ledger.sql`** borra los asientos `reference_type IN ('shipping','return')` existentes y el índice `financial_transactions_shipping_order_uniq`.
  - **Envíos del mes** (`GET /finance/reconciliation` → `shipping`): `charged` (líneas fee de envío), `orders_cost` (`SUM(shipping_cost)` por `COALESCE(shipped_at, created_at)`), `paid_ledger` (gastos manuales con categoría en `SHIPPING_PAID_CATEGORIES` = `Logística — Envío a cliente` + legado `Envios a clientes`), `net = charged − orders_cost`, `diff = paid_ledger − orders_cost`.
  - **Dashboard**: `returnsCost` ya no sale del libro; se calcula desde `/returns` por `resolved_at` en rango: `product.cost` si `scrapped` + `return_shipping_cost` si `company_assumes_shipping`.
  - **Pago de prendas**: cuenta prendas de pedidos con `delivered_at` en `[from, to)` **y** no incluidos en otro pago. **Migración `027_unit_payment_run_orders.sql`**: tabla `unit_payment_run_orders (order_id PK, run_id, units)` con backfill desde los runs existentes (el más antiguo se queda el pedido). `POST /` amarra los pedidos al run y deriva `units`/`total` de esas filas (23505 concurrente → 409); se eliminó el bloqueo por solape y `?force`. `GET /units` devuelve `already_paid_units`/`already_paid_orders`; `DELETE /:id` libera los pedidos por CASCADE. Predicado `notYetPaidSql()` en `server/lib/orderUnits.ts`.
...
- **Reglas de negocio en server**:
  ...
  - `returns/:id/resolve`: transaccional — update return + stock. **No** escribe en `financial_transactions`.
  - `finance`: el libro es **solo manual** salvo `reference_type='unit_payment'`; ningún otro flujo inserta asientos. PATCH/DELETE solo permite `reference_type='manual'`.
